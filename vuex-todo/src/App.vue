  ...
  <div id="app">
    ...
    <a-list bordered :dataSource="infoList" class="dt_list">
      <a-list-item slot="renderItem" slot-scope="item">
        <!-- 复选框 -->
        <a-checkbox :check="item.done" @change="(e)=>{ cbStatusChanged(e,item.id)}">{{item.info}}</a-checkbox>
        <!-- 删除链接 -->
...
export default {
  ...
  methods: {
    ...
    cbStatusChanged (e, id) {
      ...
      this.$store.commit('handleStatusChange', param)
    },
    // 清除已完成的
    clean () {
      this.$store.commit('cleanDoneItem')
    },
    changeList (key) {
